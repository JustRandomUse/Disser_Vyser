...
export default {
  ...
  computed: {
    statistics() {
      ...
      params.forEach(param => {
        const values = this.sensors.map(s => s[param] || 0).filter(v => v > 0);
        if (values.length > 0) {
  ...
  methods: {
    ...
    renderChart() {
      ...
      const params = Object.keys(this.statistics);
      const candlestickData = params.map(p => {
        const stat = this.statistics[p];
        // Candlestick format: [open, close, lowest, highest]
        // We use: [min, max, min, avg] to show range with average
